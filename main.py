...
def convertFromFormat1(jsonObject):
    ...
    # Check if the timestamp is already an integer/float (Unix Epoch)
    # or if it's a string that needs parsing
    ts_raw = jsonObject['timestamp']
    
    if isinstance(ts_raw, (int, float)):
        # If it's already a number, we just ensure it's in milliseconds
        # Note: If your input is seconds, multiply by 1000. 
        # If it's already ms, leave it. Assuming it's seconds based on common errors.
        timestamp = int(ts_raw * 1000) if ts_raw < 1e12 else int(ts_raw)
    else:
        # It's a string, so we parse it
